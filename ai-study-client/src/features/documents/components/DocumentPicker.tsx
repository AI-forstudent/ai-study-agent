        ...
        <div className="absolute top-full mt-2 end-0 w-64 bg-white rounded-xl shadow-lg border border-[#E8E8E6] z-[100] overflow-hidden">
          {docs.length === 0 ? (
            ...
            <ul className="max-h-72 overflow-y-auto py-1">
              {docs.map(doc => {
                const { Icon: DocTypeIcon } = getFileIconConfig(doc.title);
                return (
                <li
                  key={doc.id}
                ...
                >
                  <DocTypeIcon className={`w-4 h-4 flex-shrink-0 ${doc.id === selectedId ? 'text-indigo-600' : 'text-[#C4C4C4]'}`} />
                  <span
                    className={`flex-1 text-sm truncate ${doc.id === selectedId ? 'font-semibold text-[#37352F]' : 'text-[#787774]'}`}
                  >
                    {doc.title}
                  </span>
                  <button
