...
**`backend/app/api/routers/lectures.py`** — Mounted at `/api/v1` (declares full paths, same dual-mount pattern as exams). Routes: `GET /courses/{course_id}/lectures` (list — any course reader, sorted by `lecture_date DESC NULLS LAST`), `POST /courses/{course_id}/lectures` (owner-only create), `GET /lectures/{id}` (detail), `PUT /lectures/{id}` (partial update — uses Pydantic's `model_fields_set` to distinguish "field absent" from "explicit clear" so attachment FKs and `lecture_date` can each be cleared individually), `DELETE /lectures/{id}` (owner-only; recording + notes UserDocuments are NOT deleted — only the lecture row goes away), **`POST /lectures/{id}/unified-summary`** (F-033 — owner-only; kicks off async unified-summary generation via FastAPI BackgroundTask, returns 202 with `unified_summary_processing=true`, frontend polls), **`GET /lectures/{id}/threads`** (F-033 — caller's chat threads pinned to this lecture, root threads only with full message lists). F-031 Phase 1 + F-033 — Phase 2 will add `/lectures/{id}/transcribe` (Gemini audio) feeding the same unified-summary generator. Depends on `get_current_user`.
...
**`backend/app/services/prompt_builder.py`** — `resolve_system_prompt(persona_id, db, course_id=None, lecture_id=None)`. Four-layer composition: persona base prompt (`Persona.system_prompt` → `Persona.manual_prompt_override` → built-in default) → SessionMemory blocks (chronological) → optional `## COURSE CONTEXT` block from the course's cached syllabus extraction → optional `## LECTURE CONTEXT` block from the lecture's cached `unified_summary` (falling back to `lecturer_summary`, capped at 8K chars). The lecture block is added in F-033; chat router pulls the lecture_id off the Thread row each turn so the assistant grounds answers in the active lecture's material.
...
**`backend/app/services/lecture_summary_generator.py`** — F-033. Hardcodes the locked Hebrew unified-summary skill (system prompt + user prompt template + edge cases) — a six-input contract: lecturer_summary, recording_transcript, notes_text, exercises_text, homework_text, plus lecture/course titles. `process_unified_summary_background(lecture_id, user_id)` is the FastAPI BackgroundTask entrypoint — opens its own DB session, runs the generator through `call_llm_with_usage` (writes a `UsageEvent` with endpoint `lecture_unified_summary`), and persists either `unified_summary` (success) or `unified_summary_error` (failure). Last-good `unified_summary` is preserved on failure. Notes text is pulled from the lecture's `notes_user_document_id` CAS chunks when the attachment is a PDF; images are skipped (no OCR yet). Phase 2 will populate `recording_transcript` via Gemini audio transcription and feed it into the same generator.
...
**`backend/app/models/domain.py`** — Single source of truth for all ORM models. Includes: `User` (with `subscription_tier`, `auth_provider`, `google_sub`, **`home_organization_id`** [Phase 1]), **`Organization`** [Phase 1], **`Community`** [Phase 1], **`RoleAssignment`** [Phase 1, with platform-NULL-sentinel CHECK constraint], `BaseDocument`, `UserDocument` (+ Phase-1 `organization_id` denormalized scope), `Folder` (with optional `course_id`, + Phase-1 `community_id` / `organization_id`), `Course` (with `syllabus_user_document_id`, cached `syllabus_extracted` JSONB, + Phase-1 NOT-NULL `community_id` / `organization_id`), `CourseMembership` (legacy — Phase 6 will drop in favour of `RoleAssignment`), `CourseTopic`, `CourseLecturer`, `CourseQuestionType`, `Exam` (+ Phase-1 NOT-NULL `community_id` / `organization_id`), `ExamQuestion`, plus the `exam_lecturers` and `exam_question_topics` association tables, **`Lecture`** (F-031 + F-033 — title, lecture_date, `lecturer_summary` [authoritative skill input, renamed from manual_summary in migration u6t7s8r9q0p1], `student_summaries` [peer summaries, ignored by skill], `unified_summary` + `unified_summary_processing` + `unified_summary_error` + `unified_summary_generated_at` [AI-generated cache], recording + notes FKs to UserDocument with ON DELETE SET NULL, + Phase-1 NOT-NULL `community_id` / `organization_id`), `Thread` (with `user_id`, `course_id`, **`lecture_id`** [F-033, FK→lectures.id ON DELETE SET NULL], + Phase-1 `community_id` / `organization_id`), `Message`, `Chunk`, `PageSummary`, `Persona`, `StudySession`, `SessionMemory`, `UserProfile`, `CourseRecord`, `JobApplication`, `LectureVideo`, `VideoSyncIndex`, `UsageEvent`. Three native PG enums declared at module top: `SCOPE_ENUM` / `ROLE_ENUM` / `GRANT_SOURCE_ENUM` (Phase 1). Alembic autogenerates migrations from this file.
